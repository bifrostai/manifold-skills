...
suggested next skill is `/wrap-policy`, then `/containerize-wrap`.
...
policy slug, registry, benchmarks, the detected package manager, GPU
memory, and the weights location.
...
than "what is your peak VRAM in GB?". Ask "where do you
usually run this model?" rather than "what is the deployment style?". If
...
The next skill tests the wrap by loading the model on this machine.
...
- If this machine cannot run the model, ask before continuing. Give
  ...
  again there. `/wrap-policy` reads that field back, skips its test
  run, and repeats the warning in its handoff.
- If this machine cannot build the image, flag that too. The
  containerize step needs Docker and 10 to 30 GB of free disk. On an
...
- Any prerequisites Phase 1 found missing that a later skill will
  need. Say so if Docker is not installed or not reachable. Give the
  user two more facts when they apply. If this machine cannot run the model, `/wrap-policy` will
...
If the user says yes, invoke `/wrap-policy`. If the user says no, or
